        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="flex items-center gap-2 text-primary mb-4">
              ...
              <span className="text-sm font-medium">Lesson {lessonId} of 7</span>
              {isCompleted && (<><span className="text-muted-foreground">•</span><span className="flex items-center gap-1 text-success text-sm"><CheckCircle className="w-4 h-4" />Completed</span></>)}
            </div>
          ...
          {lesson.downloadables && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-muted/30 rounded-xl p-4 mb-8">
              <p className="font-semibold mb-3 flex items-center gap-2"><Download className="w-5 h-5 text-primary" />Downloadable Materials</p>
              <div className="flex flex-wrap gap-3">
                <button onClick={() => alert('Downloading PDF')} className="flex items-center gap-2 px-4 py-2 bg-background rounded-lg hover:bg-muted transition-colors text-sm"><FileText className="w-4 h-4 text-primary" />Download PDF</button>
                <button onClick={() => alert('Downloading Checklist')} className="flex items-center gap-2 px-4 py-2 bg-background rounded-lg hover:bg-muted transition-colors text-sm"><FileText className="w-4 h-4 text-success" />Download Checklist</button>
                {lesson.downloadables.protocol && <button onClick={() => alert('Downloading Protocol')} className="flex items-center gap-2 px-4 py-2 bg-background rounded-lg hover:bg-muted transition-colors text-sm"><FileText className="w-4 h-4 text-warning" />30-Day Protocol</button>}
              </div>
            </motion.div>
          )}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row gap-4 mt-12 pt-8 border-t border-border">
            {prevLessonId && <button onClick={() => navigate(`/modulo3/lesson/${prevLessonId}`)} className="btn-secondary flex items-center justify-center gap-2 flex-1"><ArrowLeft className="w-4 h-4" />Previous Lesson</button>}
